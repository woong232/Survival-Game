...
public class WeaponSway : MonoBehaviour
{
    ...
    {
        if (Input.GetAxisRaw("Mouse X") != 0 || Input.GetAxisRaw("Mouse Y") != 0) // 움직였을떄
            Swaying();
        else
            BackToOriginPos();
    }
    ...
    {
        float _moveX = Input.GetAxisRaw("Mouse X"); // 마우스 움직임 값
        float _moveY = Input.GetAxisRaw("Mouse Y"); // 마우스 움직임 값
